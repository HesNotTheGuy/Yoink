...
$launchCmd += "`r`n"
$launchCmd += "REM Check if Yoink (specifically) is already running on port $Port.`r`n"
$launchCmd += "REM Pipe through findstr so we don't false-positive on any other app`r`n"
$launchCmd += "REM that happens to respond on the same port (Vite, another Next dev`r`n"
$launchCmd += "REM server, Idea Map, etc.). /api/ping returns JSON containing 'yoink'.`r`n"
$launchCmd += "curl -s -m 2 http://localhost:$Port/api/ping 2>nul | findstr /C:`"yoink`" >nul 2>&1`r`n"
