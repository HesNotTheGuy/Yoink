...
// Advanced options
const filenameTpl   = $('filenameTpl');
const subsEnabled   = $('subsEnabled');
const subsOptions   = $('subsOptions');
const subsLang      = $('subsLang');
const cookiesEnabled = $('cookiesEnabled');

// Toggle subtitle options visibility
...
// ---------------------------------------------------------------------------
// Cookies → Netscape cookie file format (what yt-dlp expects)
// ---------------------------------------------------------------------------
async function getCookiesNetscape(url) {
  if (!chrome.cookies || !url) return null;
  try {
    ...
    for (const c of cookies) {
      ...
      lines.push([domain, includeSubdomains, cpath, secure, expires, c.name, c.value].join('\t'));
    }
    return lines.join('\n');
  } catch (e) {
    console.warn('[Yoink] cookies read failed:', e);
    return null;
  }
}

// ---------------------------------------------------------------------------
// Subtitle options bundle
// ---------------------------------------------------------------------------
function getSubtitleOpts() {
  if (!subsEnabled.checked) return null;
  const mode = document.querySelector('input[name="subsMode"]:checked');
  return {
    enabled: true,
    lang: (subsLang.value || 'en').trim(),
    embed: !mode || mode.value === 'embed'
  };
}
...
dlBtn.addEventListener('click', async () => {
  ...
  const cookies = cookiesEnabled.checked ? await getCookiesNetscape(currentUrl) : null;
  const subtitles = getSubtitleOpts();
  const filenameTemplate = (filenameTpl.value || '').trim() || null;
